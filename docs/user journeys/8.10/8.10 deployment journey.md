...
3. **Extend your deployment** — add high availability architectures, more tenants, or process analytics with Optimize, in any order, at any time.
4. **Day-2 operations** — upgrades, backups, monitoring, scaling, onboarding new teams.
...
- **CockroachDB** has the best comparison-table pattern for helping customers choose between resilience options — adopted for High availability architectures.
...
| Multi-region buried as one page under Concepts | First-class High availability architectures section with three named tiers |
| Backup & restore buried under Concepts | First-class Day-2 entry |

The 8.10 shape also makes deployment a *living* artifact rather than a fixed topology. The Extend section's three areas (High availability architectures, Tenancy & isolation, Process analytics with Optimize) can be adopted independently, in any order, at any time after the Baseline is healthy. That matches how customers actually evolve their deployments — and none of the three competitors models it explicitly.
...
4. **`helm/operational-tasks/dual-region-operational-procedure` is in Operational tasks** but it's really High availability architectures → Tier 2 content. It's misfiled because today's sidebar has no High availability architectures section to host it.
...
- **"Choose your tier" comparison table** as the opening of High availability architectures (Tier framework + RPO/RTO) — see Appendix A for the proposed Camunda version.
...
│   │   ├─ Choose your topology                                 [Kubernetes] [Containers] [Manual]
│   │   ├─ PostgreSQL / RDBMS
│   │   │   ├─ Setup guide
...
│   ├─ Production readiness checklist                           [Kubernetes] [Containers] [Manual]
│   └─ Smoke test & exit criteria
│
├─ 3. Extend your deployment
│   ├─ Overview — when to extend
│   ├─ High availability architectures                          [Kubernetes] [Containers] [Manual]
...
│   └─ Process analytics with Optimize
...
├─ 4. Day-2 operations
│   ├─ Overview
│   ├─ Upgrades                                                 [Kubernetes] [Containers] [Manual]
│   ├─ Backups & restore
│   │   ├─ Overview
│   │   ├─ Elasticsearch / OpenSearch
│   │   │   ├─ Backup
...
| Current path | New location | Action |
|---|---|---|
| `self-managed/reference-architecture/reference-architecture` | `1. Plan → Choose your deployment target` landing | **Rewrite** — see Appendix B |
| `self-managed/reference-architecture/kubernetes` | `1. Plan → Choose your deployment target → Kubernetes` | **Rewrite** — see Appendix B (trim) |
| `self-managed/reference-architecture/containers` | `1. Plan → Choose your deployment target → Containers` | Move |
| `self-managed/reference-architecture/manual` | `1. Plan → Choose your deployment target → Manual / VM` | Move |
| — (NEW) | `3. Extend → High availability architectures → Tier 2 → Multi-region concept` | **New** — see Appendix A |
| — (NEW) | `3. Extend → High availability architectures → Tier 3 — Three-region stretched` | **New** — see Appendix A |
| — (NEW) | `3. Extend → Tenancy & isolation → Physical tenants → Concept` | **New** — see Appendix A |

### 6.3 Deploy and manage → Helm install pages

| Current path | New location | Action |
|---|---|---|
| `self-managed/deployment/index` | Dissolved — paradigm choice lives in `1. Plan → Choose your deployment target` | **Delete** (with redirect) |
...
| `self-managed/deployment/helm/install/helm-with-rdbms` | `2. Baseline → 2. Provision databases → PostgreSQL / RDBMS → Example end-to-end install` | **Move** |
...
| `.../configure/operator-based-infrastructure` | `1. Plan → Database options → Kubernetes operators` | **Rewrite** — see Appendix B |
...
| `.../configure/database/rdbms` | `2. Baseline → 2. Provision databases → PostgreSQL / RDBMS` landing | Move |
| `.../configure/database/rdbms-jdbc-drivers` | `… → PostgreSQL / RDBMS → JDBC drivers` | Move |
| `.../configure/database/rdbms-search-and-result-limits` | `… → PostgreSQL / RDBMS → Search & result limits` | Move |
| `.../configure/database/rdbms-schema-management` | `… → PostgreSQL / RDBMS → Schema management` | Move |
| `.../configure/database/rdbms-troubleshooting` | `… → PostgreSQL / RDBMS → Troubleshooting` | Move |
| `.../configure/database/validate-rdbms` | `… → PostgreSQL / RDBMS → Validate setup` | Move |
| `.../configure/database/access-sql-liquibase-scripts` | `… → PostgreSQL / RDBMS → Liquibase scripts` | Move |
| `.../configure/database/non-sql` | `2. Baseline → 2. Provision databases → Operating Elasticsearch / OpenSearch` landing | **Rewrite** — see Appendix B |
| `.../configure/database/elasticsearch/using-external-elasticsearch` | `2. Baseline → 2. Provision databases → Splitting databases per component → External Elasticsearch for OC` | Move |
| `.../configure/database/using-external-opensearch` | `… → Splitting databases per component → External OpenSearch for OC` | Move |
| `.../configure/database/configure-db-custom-headers` | `… → Elasticsearch / OpenSearch → Custom headers` | Move |
| `.../configure/database/elasticsearch/prefix-elasticsearch-indices` | `… → Elasticsearch / OpenSearch → Index prefixes` | Move |
| `.../configure/database/all-shards-failed` | `… → Elasticsearch / OpenSearch → Troubleshooting` | Move |
| `.../configure/database/using-existing-postgres` | `… → Splitting databases per component → Hub Postgres on its own instance` | Move |
| `.../configure/database/optimize/index` | `3. Extend → Process analytics with Optimize → Storage requirements` | **Merge** |
...
| `.../operational-tasks/migration-from-bitnami/index` | `Migrate from 8.9 → 8.10 → Migrate off Bitnami subcharts` landing | Move |
...
| `.../operational-tasks/dual-region-operational-procedure` | `3. Extend → High availability architectures → Tier 2 → Operational procedure` | **Rewrite** — see Appendix B |
...
| `.../amazon-eks/dual-region` | `3. Extend → High availability architectures → Tier 2 → AWS EKS` | **Move** + rewrite intro |
...
| `.../amazon/openshift/terraform-setup-dual-region` | `3. Extend → High availability architectures → Tier 2 → ROSA on AWS` | Move |
...
| `.../openshift/redhat-openshift-dual-region` | `3. Extend → High availability architectures → Tier 2 → Red Hat OpenShift` | Move |
...
| Current path | New location | Action |
|---|---|---|
| `self-managed/concepts/authentication/authentication-to-orchestration-cluster` | `1. Plan → Identity & authentication → Authentication concepts` | **Rewrite** — see Appendix B |
| `self-managed/concepts/authentication/authentication-to-management-components` | `1. Plan → Identity & authentication → Authentication concepts` (sibling sub-page) | **Rewrite** — see Appendix B |
| `self-managed/concepts/secondary-storage/index` | `2. Baseline → 2. Provision databases → Secondary storage concepts` | **Merge** with other secondary-storage pages |
...
| `self-managed/concepts/secondary-storage/rdbms-benchmark-results` | `2. Baseline → 2. Provision databases → PostgreSQL / RDBMS → Benchmark results` | Move |
| `self-managed/concepts/databases/overview` | `1. Plan → Database options → RDBMS vs ES/OS — comparison` | **Merge** |
| `self-managed/concepts/databases/elasticsearch/elasticsearch-privileges` | `2. Baseline → 2. Provision databases → Elasticsearch / OpenSearch → Privileges` | Move |
| `… elasticsearch-without-cluster-privileges` | `… → Elasticsearch / OpenSearch → Privileges` (sub-page) | Move |
| `… opensearch-privileges` | `… → Elasticsearch / OpenSearch → Privileges` (sub-page) | Move |
| `… opensearch-without-cluster-privileges` | `… → Elasticsearch / OpenSearch → Privileges` (sub-page) | Move |
| `self-managed/concepts/databases/relational-db/index` | `2. Baseline → 2. Provision databases → PostgreSQL / RDBMS` landing | Move |
| `.../databases/relational-db/rdbms-setup-guide` | `… → PostgreSQL / RDBMS → Setup guide` | Move |
| `.../databases/relational-db/database-configuration` | `… → PostgreSQL / RDBMS → Database configuration` | Move |
| `.../databases/relational-db/rdbms-support-policy` | `… → PostgreSQL / RDBMS → Support policy` | Move |
...
| `self-managed/concepts/exporters` | `2. Baseline → 2. Provision databases → Exporters` | Move |
...
| `self-managed/concepts/multi-region/dual-region` | `3. Extend → High availability architectures → Tier 2 → Multi-region concept` | **Rewrite** — see Appendix B |
...
- Build out **High availability architectures** as a unified section with the CockroachDB-style tier table and the moved Tier 2 per-target pages (EKS dual-region, OpenShift dual-region, ROSA dual-region, ECS-coming placeholder).
...
- **Dissolve `Concepts` entirely.** Move all `concepts/databases/*` and `concepts/secondary-storage/*` content into `2. Provision databases`. Move authentication concepts into Plan → Identity & authentication.
- **Build out Provision databases' PostgreSQL / RDBMS and Elasticsearch / OpenSearch sub-trees** by consolidating today's scattered DB pages.
- **Build out High availability architectures' Tier 2 / Tier 3 variants** (per the journey doc's open items).
...
**Choose your deployment target** (`1. Plan → Choose your deployment target`)
Combined topology overview and paradigm picker. Opens with the one-sentence topology statement: *One Hub, one or more Orchestration Clusters, one or more Physical Tenants per OC, and zero or more Optimize deployments per PT* — plus a simple SVG topology diagram. Then the paradigm picker: three choices (Kubernetes with Helm — recommended for production; Containers; Manual / VM) with a short trade-off table. Recommendation paragraph: "Kubernetes with Helm is the recommended path for production." This choice persists via Docusaurus tab `groupId="deployment-paradigm"`. Sub-pages: Kubernetes, Containers, Manual / VM — each containing the architecture diagram for that paradigm. Links forward to Build your Baseline.

**Database options — landing** (`1. Plan → Database options`)
...
**Identity & authentication — landing** (`1. Plan → Identity & authentication`)
...
Frames database provisioning as a hard prerequisite before any Helm install (no in-chart Bitnami fallback). Lists the three dependency types: PostgreSQL/RDBMS, Elasticsearch/OpenSearch, Keycloak. Suggests order: provision DB → provision IdP → run Helm. Cross-links to Migrate from 8.9 → 8.10 → Migrate off Bitnami subcharts for customers upgrading from 8.9.
...
Frames Extend as additive, optional, and orderable in any sequence. Three extension areas defined: High availability architectures, Tenancy & isolation, Process analytics with Optimize. Adoption signals table: "If you need X, look at Y." Cross-link to Before you install: future-proofing checklist so customers can see whether they've planned ahead.

**High availability architectures — Tier framework + RPO/RTO** (`3. Extend → High availability architectures → Tier framework + RPO/RTO`)
...
**Process analytics — Add Optimize per Physical Tenant** (`3. Extend → Process analytics with Optimize → Add Optimize per Physical Tenant`)
...
Step-by-step: for each Optimize instance, register a dedicated Mgmt Identity application; configure the app's redirect URIs and secrets; wire the secrets into the Optimize values file. Calls out that N PTs with Optimize = N Mgmt Identity app registrations. Cross-link to Plan → Identity & authentication.
...
**Choose your topology** (`2. Baseline → 2. Provision databases → Choose your topology`)
...
**Splitting databases per component — landing** (`2. Baseline → 2. Provision databases → Splitting databases per component`)
...
Frames Day-2 as the ongoing operations of a healthy deployment. Lists the major operational concerns: upgrades, backups, monitoring, scaling, onboarding new teams, credentials rotation, diagnostics, troubleshooting. Cross-link to Migrate from 8.9 → 8.10 for the once-per-version upgrade event.
...
**DR drills (per-tier failover/failback)** (`3. Extend → High availability architectures → DR drills`)
...
Currently a list of deployment options (Helm, Docker, Manual). Delete with redirect to `1. Plan → Choose your deployment target → Choose your paradigm`. Paradigm choice is now made once in Plan and persists via Docusaurus tab `groupId`; a second picker page in Baseline is redundant.
...
**`helm/configure/database/non-sql` — Elasticsearch / OpenSearch (in Provision databases)**
Currently a configure-time page. Rewrite as the ES/OS setup landing page in 2. Baseline → 2. Provision databases → Elasticsearch / OpenSearch. Rename "non-sql" to "Elasticsearch / OpenSearch" throughout. Reframe content from "how to point at an ES/OS at install time" to "how to provision and configure an ES/OS-backed Camunda deployment."
...
| `helm/configure/database/non-sql` | `baseline/provision-databases/elasticsearch-opensearch` | Friendlier slug; new location |
| `helm/configure/operator-based-infrastructure` | `plan/database-options/kubernetes-operators` | New location |
| `helm/configure/configure-multi-tenancy` | `extend/tenancy-isolation/logical-tenants` | Reframed scope |
| `concepts/multi-region/dual-region` | `extend/high-availability-architectures/tier-2/multi-region-concept` | New location |
...
- `reference/supported-environments` — cross-link from Plan → Choose your deployment target and Helm v4 requirements.
