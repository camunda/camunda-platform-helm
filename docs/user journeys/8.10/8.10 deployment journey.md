...
│   ├─ Install
│   │   ├─ Choose your install paradigm
│   │   ├─ Kubernetes with Helm
│   │   │   ├─ 1. Provision your Kubernetes cluster
│   │   │   │   ├─ AWS EKS
│   │   │   │   ├─ Azure AKS
│   │   │   │   ├─ Google GKE
│   │   │   │   └─ Red Hat OpenShift
│   │   │   │       ├─ OpenShift (IPI / bare-metal)
│   │   │   │       └─ ROSA on AWS
│   │   │   ├─ 2. Install Camunda with Helm
│   │   │   │   ├─ Install Camunda with Helm        (3 overview pages merged into 1)
│   │   │   │   ├─ Helm v4 requirements
│   │   │   │   └─ Chart parameters
...
│   │   ├─ Containers
│   │   │   ├─ Provision your container infrastructure
│   │   │   │   ├─ Docker
│   │   │   │   └─ AWS ECS
│   │   │   └─ Container-specific configuration
│   │   └─ Manual / VM
│   │       ├─ Provision your VM (AWS EC2 manual)
│   │       └─ Install Camunda manually
│   │           ├─ Manual install
│   │           └─ Manual with RDBMS
...
│   │   └─ Document handling — storage configuration
...
| Current path | New location | Action |
|---|---|---|
| `self-managed/reference-architecture/reference-architecture` | `1. Plan → Decide: deployment target → Reference architectures` landing | **Rewrite** — see Appendix B |
| `self-managed/reference-architecture/kubernetes` | `1. Plan → … → Kubernetes blueprint → Single-region` | **Rewrite** — see Appendix B (trim) |
| `self-managed/reference-architecture/containers` | `1. Plan → … → Containers blueprint` | Move |
| `self-managed/reference-architecture/manual` | `1. Plan → … → Manual / VM blueprint` | Move |
| — (NEW) | `1. Plan → … → Kubernetes blueprint → Dual-region (Tier 2)` | **New** — see Appendix A |
| — (NEW) | `1. Plan → … → Kubernetes blueprint → Three-region (Tier 3)` | **New** — see Appendix A |
| — (NEW) | `1. Plan → … → Kubernetes blueprint → Multi-PT variant` | **New** — see Appendix A |

### 6.3 Deploy and manage → Helm install pages

| Current path | New location | Action |
|---|---|---|
| `self-managed/deployment/index` | `2. Baseline → Install → Choose your install paradigm` | **Rewrite** — see Appendix B |
| `self-managed/deployment/helm/index` | `2. Baseline → Install → Kubernetes with Helm → 2. Install Camunda with Helm → Install Camunda with Helm` | **Merge** (3-into-1) — see Appendix B |
...
| `self-managed/deployment/helm/chart-parameters` | `2. Baseline → Install → Kubernetes with Helm → 2. Install Camunda with Helm → Chart parameters` | Move |
...
**Deployment-paradigm badges on cross-cutting pages.** A small `Helm only` / `ECS only` / `All paradigms` badge in the page header on every page that isn't paradigm-agnostic. Resolves the SUPPORT-29189 class of issue (feature tagged for both SaaS and SM but only applying to SM, with the restriction buried).
...
- **Restructure Helm install** to put cluster provisioning first, install second, K8s-specific config third (the most impactful single change for K8s admins).
- **Merge the three Helm install overview pages** into one.
...
Frames the identity decision as the second Phase 0 decision. Introduces the choice surface: external OIDC for production (Entra ID, Okta, …) vs basic auth for dev/eval. Notes Mgmt Identity stays as a separate component in 8.10, planned removal in 8.11. Links to Authentication concepts and Mgmt Identity vs OC Admin pages.
...
**Reference architecture — Kubernetes blueprint → Three-region (Tier 3)** (`… → Three-region (Tier 3)`)
...
**Reference architecture — Kubernetes blueprint → Multi-PT variant** (`… → Multi-PT variant`)
...
**`self-managed/reference-architecture/reference-architecture` — Reference architectures landing**
Currently a thin page introducing the three reference architectures. Rewrite as the picker page used during Plan → Decide deployment target. Add: per-paradigm trade-off table; explicit positioning as architecture blueprints (not step-by-step install guides — those live in Baseline → Install); cross-links forward to per-paradigm subpages.

**`self-managed/reference-architecture/kubernetes` — Kubernetes single-region blueprint**
Currently a single beefy page covering single-region K8s topology. Trim: keep the single-region topology content; move any references to install steps out (those live in Baseline → Install now); add cross-links to the three new Kubernetes variant pages (Dual-region, Three-region, Multi-PT). Goal: this page becomes the canonical single-region blueprint, three new pages join it as siblings.
...
**Helm install overview — 3-into-1 merge** (`helm/index` + `helm/install/index` + `helm/install/production/index`)
Three current overview pages that all introduce Helm install at slightly different levels become one. Merged structure: Helm v4 prerequisite → What the chart deploys (Hub + OC as two releases) → Quick install vs production install side by side (absorbing today's `production/index` content) → Per-target infrastructure (table of links forward) → Chart parameters reference link. Redirect both deprecated URLs to the unified page.
