...
│   ├─ 5. Configure Camunda                                     [Kubernetes] [Containers] [Manual]
...
| `self-managed/deployment/helm/chart-parameters` | `2. Baseline → 5. Configure Camunda → Chart parameters` `[Kubernetes]` | Move |

### 6.4 Deploy and manage → Helm Configure pages

| Current path | New location | Action |
|---|---|---|
| `self-managed/deployment/helm/configure/index` | Dissolved — no "Common configuration" landing page in new structure | **Delete** (with redirect to `2. Baseline → 5. Configure Camunda`) |
| `.../configure/application-configs` | `2. Baseline → 5. Configure Camunda → Application config vs deployment-tool values` | **Rewrite** (boost prominence) — see Appendix B |
...
| `.../configure/secret-management` | `2. Baseline → 5. Configure Camunda → Secret management` | Move |
| `.../configure/running-custom-connectors` | `2. Baseline → 5. Configure Camunda → Custom connectors` | Move |
...
| `.../configure/license-key` | `2. Baseline → 5. Configure Camunda → License key` | Move |
