...
- **Privileged, public-API, and agent-instruction paths always need a human** — any PR whose net diff (including renamed-from paths) touches a CI-privileged path (`.github/workflows/`, `.github/actions/`, `CODEOWNERS`, the allowlist, the auto-approve gate implementation under `scripts/auto-approve-gate/`), a chart's public API (`values.yaml`, `values.schema.json`, `constraints.tpl`; chart `test/` fixtures excluded), or an agent instruction file (`AGENTS.md`/`CLAUDE.md`/`SKILLS.md`, `.claude/`, `.github/instructions/`, `.github/copilot-instructions.md`, `.github/escalation-policy.md`) is never auto-approved. The exact set lives in [`.github/auto-approve-protected-paths.txt`](https://github.com/camunda/camunda-platform-helm/blob/main/.github/auto-approve-protected-paths.txt).
- **Renovate PRs use a narrower protected set** — `renovate[bot]`-authored PRs are auto-approved on every push but may never touch the approval trust machinery (the allowlist, the protected-path lists, the auto-approve workflow, `CODEOWNERS`, `scripts/auto-approve-gate/`), per [`.github/auto-approve-protected-paths-renovate.txt`](https://github.com/camunda/camunda-platform-helm/blob/main/.github/auto-approve-protected-paths-renovate.txt). Approval additionally requires every triggering push to come from `renovate[bot]` or `distro-ci[bot]` — a push by anyone else drops the PR to human review. Renovate's routine changes (workflow action pins, chart image tags) remain governed by the `renovate.json5` automerge policy.
- **Fail-closed** — if the workflow cannot determine what a PR changed, it does not approve.
- **Stale approvals are self-dismissed** — when a new head is not auto-approvable, the workflow dismisses its own earlier approvals, so a previously approved PR cannot merge on a stale approval after an unapprovable push. Because `distro-ci[bot]` and `github-actions[bot]` also approve on behalf of unrelated automation (for example the release-please auto-merge in `chart-release-public.yaml`), approvals from those identities are only dismissed when the review body matches one this gate wrote; `renovate-approve[bot]`'s one-shot approvals are always dismissed.
