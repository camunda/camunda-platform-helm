...
- **Stale approvals are self-dismissed** — when a new head is not auto-approvable, the workflow dismisses its own earlier bot approvals (`distro-ci[bot]`, `github-actions[bot]`), so a previously approved PR cannot merge on a stale approval after an unapprovable push.
