...
**NEVER hand-edit `Chart.yaml`.** The `artifacthub.io/changes` block is **autogenerated** at release time by `scripts/camunda-core/pkg/releasenotes` (`ArtifactHubChanges`), which parses the git-cliff `RELEASE-NOTES.md` sections — themselves derived from Conventional-Commit PR titles — and merges the result into `Chart.yaml` via `yq`. Only the `Features`→`added`, `Refactor`→`changed`, and `Fixes`→`fixed` groups produce entries; `Documentation` and `Revert` are dropped, so a `docs:`/`revert:` PR yields no entry regardless. Do not add, edit, or remove `artifacthub.io/changes` entries by hand in any PR — the PR title is the only input, and manual edits are overwritten. Leave `Chart.yaml` untouched.
