...
**Tier split:** `pull_request` runs **tier-1 only** — `test-chart-version.yaml` passes `tier: 1` on PR events. The **full matrix** (tier-2 plus any untiered scenarios) runs on `merge_group` (merge queue), which applies no tier filter at all. A scenario that declares no `tier:` matches **no** `--tier` filter, so it is reachable only through the unfiltered merge-queue matrix and is invisible to `matrix list --tier 1|2` — always give an enabled scenario an explicit tier. A PR that adds or changes a tier-2 scenario gets no CI signal until merge is clicked; validate tier-2 changes locally before merge — see the `rfr-validation` skill (`.claude/skills/rfr-validation/SKILL.md`).

Scenarios and tiers live in the composable registry `test/ci/registry/manifest.yaml` (8.6 predates the registry, keeps the legacy inline `test/ci-test-config.yaml`, and has no active CI — manual `workflow_dispatch` only). Use `deploy-camunda matrix list --tier 2 --versions <v>` to re-derive the current set regardless of source, and plain `matrix list --versions <v>` for the full merge-queue set.
