...
PRIMARY_SHOULD_EXIT=true
[[ -n "$ADDITIONAL_PLAYWRIGHT_PROJECT" ]] && PRIMARY_SHOULD_EXIT=false
run_playwright_tests "$TEST_SUITE_PATH" "$SHOW_HTML_REPORT" "$SHARD_INDEX" "$SHARD_TOTAL" "blob" "$TEST_EXCLUDE" "$RUN_SMOKE_TESTS" "$PLAYWRIGHT_DEBUG" "$NAMESPACE" "$KUBE_CONTEXT" "$RERUN_CMD" "$IS_AUTH0" "$PLAYWRIGHT_PROJECT" "$PRIMARY_SHOULD_EXIT"
if [[ -n "$ADDITIONAL_PLAYWRIGHT_PROJECT" ]]; then
  run_playwright_tests "$TEST_SUITE_PATH" "$SHOW_HTML_REPORT" "$SHARD_INDEX" "$SHARD_TOTAL" "blob" "$TEST_EXCLUDE" "false" "$PLAYWRIGHT_DEBUG" "$NAMESPACE" "$KUBE_CONTEXT" "$RERUN_CMD" "$IS_AUTH0" "$ADDITIONAL_PLAYWRIGHT_PROJECT"
fi
