	...
	if opts.LogDir != "" {
		...
		if deployLog, err := os.Create(deployLogPath); err != nil {
			...
			ctx = executil.ContextWithBuffer(ctx, func(level, line string) {
				writeEntryLog(strings.ToUpper(level), line)
				switch level {
				case "warn":
					logging.Logger.Warn().Msg(logging.BuildPrefix(logging.FieldsFromContext(ctx), "") + line)
				default:
					logging.Logger.Info().Msg(logging.BuildPrefix(logging.FieldsFromContext(ctx), "") + line)
				}
			})
		}
	}
