...
func registerSelectionCompletion(cmd *cobra.Command) {
	...
	_ = cmd.RegisterFlagCompletionFunc("persistence", func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		scenarioPath := resolveScenarioPath(cmd)
		defaultPersistence := []string{"elasticsearch", "elasticsearch-external", "elasticsearch-self-signed", "elasticsearch-external-self-signed", "no-elasticsearch", "opensearch", "opensearch-embedded", "opensearch-external", "opensearch-self-signed", "rdbms", "rdbms-external", "rdbms-oracle", "rdbms-self-signed"}
