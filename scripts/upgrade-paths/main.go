...
func main() {
	...
	for _, name := range archetypes {
		...
		if errors.As(err, &na) {
			report.NotApplicable = append(report.NotApplicable, NotApplicable{
				Path:   name,
				Reason: na.Reason(),
			})
			continue
	...
	// A transition where every archetype was skipped would otherwise render as
	// a clean report with an empty table -- the vacuous pass this harness
	// exists to prevent. Treat it as a fixture error.
	if len(report.Results) == 0 {
		fatal(fmt.Errorf("no archetype applies to %s-to-%s: %d skipped as not applicable; "+
			"either a layer name is wrong or this transition has no fixtures",
			*from, *to, len(report.NotApplicable)))
	}
