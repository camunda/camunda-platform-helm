...
// NotApplicableError reports that an archetype cannot be expressed in the
// source chart, because a values layer it is built from does not exist there.
//
// This is deliberately not a failure. An archetype describes a deployment
// shape, and a shape that a given version cannot express is not a break in
// that version — it is a path that does not exist to be tested. Failing the
// run would let one archetype added for a later transition take down every
// earlier one, which is how 8.8-to-8.9 broke when the external shapes were
// introduced for 8.9-to-8.10.
//
// The caller is expected to report it rather than swallow it: an archetype
// silently dropped is indistinguishable from one that passed.
...
func (e *NotApplicableError) Error() string {
	return fmt.Sprintf("archetype %s cannot be expressed in chart %s: layer %q does not exist (%s)",
		e.Archetype, e.Version, e.Layer, e.Path)
}

// Reason is a short phrase for the report, without the archetype name that
// the report already puts in the adjacent column.
func (e *NotApplicableError) Reason() string {
	return fmt.Sprintf("chart %s has no `%s`", e.Version, e.Layer)
}

// LoadTransition resolves an archetype against a version pair. A baseline
// layer missing from the source chart yields a *NotApplicableError, which the
// caller should record and skip rather than treat as a failure.
