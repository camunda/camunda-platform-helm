...
# Get all enterprise images used in a certain Helm chart.
# Enterprise images are generated by overlaying values-enterprise.yaml on top of the base values.
get_chart_enterprise_images () {
    chart_version="${1}"
    major_minor="$(echo "$CHART_DIR" | sed 's/charts\/camunda-platform-//g')"
    version_matrix_file="version-matrix/camunda-$major_minor/version-matrix.json"
    enterprise_values_file="${CHART_DIR}/values-enterprise.yaml"
    test -d "${CHART_DIR}" || CHART_DIR="$(ls -d1 charts/camunda-platform-8* | tail -n1)"
    test -f "${version_matrix_file}" || echo '[]' > "${version_matrix_file}"

    # Check if enterprise values file exists for this chart version.
    if [ ! -f "${enterprise_values_file}" ]; then
      return 0
    fi
    ...
    # Check if the chart enterprise data already in version-matrix.json and add it if needed.
    if ! $(jq "any(.chart_version == \"${chart_version}\" and .chart_enterprise_images != null)" ${version_matrix_file}); then
      # Generating the chart version enterprise data.
      helm repo update > /dev/null
      # Note: --version flag only works for remote charts, not local directories
      if [[ "${is_local_chart}" == "true" ]]; then
        helm_version_arg=""
      ...
      chart_enterprise_images_json="$(echo -e "$chart_enterprise_images" | jq -R | jq -sc)"
      # Update the existing entry or add a new one with enterprise images.
      if $(jq "any(.chart_version == \"${chart_version}\")" ${version_matrix_file}); then
        output_json="$(cat ${version_matrix_file} | jq -r "map(if .chart_version == \"${chart_version}\" then . + {\"chart_enterprise_images\": ${chart_enterprise_images_json}} else . end)")"
      else
        output_json="$(cat ${version_matrix_file} | jq -r ". + [{ \"chart_version\": \"${chart_version}\", \"chart_enterprise_images\": ${chart_enterprise_images_json}}]")"
      fi
      echo "$output_json" > "${version_matrix_file}"
    fi
