...
# Generates a self-signed RSA-4096 cert for the Connectors HTTP server,
# packages it as a PKCS12 keystore with the literal password "changeit",
# and creates a `connectors-tls-keystore` Secret in the scenario namespace.
...
[req_dn]
CN = ${RELEASE}-connectors

[v3_req]
subjectAltName = @alt_names

[alt_names]
DNS.1 = ${RELEASE}-connectors
DNS.2 = ${RELEASE}-connectors.${NAMESPACE}.svc
DNS.3 = ${RELEASE}-connectors.${NAMESPACE}.svc.cluster.local
DNS.4 = ${RELEASE}-connectors-headless
DNS.5 = ${RELEASE}-connectors-headless.${NAMESPACE}.svc
DNS.6 = localhost
IP.1 = 127.0.0.1
EOF
...
create_or_replace_secret "connectors-tls-keystore" \
