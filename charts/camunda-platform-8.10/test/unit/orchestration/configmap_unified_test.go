	...
	testCases := []testhelpers.TestCase{
		...
		},
		{
			...
			Values: map[string]string{
				...
				"orchestration.clusterTopology.zones[0].priority":                   "100",
				"orchestration.clusterTopology.zones[1].name":                       "region-b",
				"orchestration.clusterTopology.zones[1].numberOfBrokers":            "2",
				"orchestration.clusterTopology.zones[1].numberOfReplicas":           "1",
				"orchestration.clusterTopology.zones[1].priority":                   "50",
				"orchestration.exporters.rdbms.enabled":                         "true",
				"orchestration.data.secondaryStorage.rdbms.url":                 "jdbc:postgresql://localhost:5432/camunda",
				"orchestration.data.secondaryStorage.rdbms.username":            "camunda",
	...
	testCases := []testhelpers.TestCase{
		{
			...
			Expected: map[string]string{
				"ERROR": "orchestration.clusterTopology.regions and orchestration.clusterTopology.regionId cannot be used with zoned mode",
			},
		},
		{
			...
			Expected: map[string]string{
				"ERROR": "orchestration.clusterSize is 6 but orchestration.clusterTopology.zones sums to 4 brokers",
			},
		},
		{
			...
			Expected: map[string]string{
				"ERROR": "orchestration.replicationFactor is 4 but orchestration.clusterTopology.zones sums to 2 replicas",
			},
		},
		{
			...
			Expected: map[string]string{
				"ERROR": "require orchestration.clusterTopology.mode=zoned",
			},
		},
		{
			...
			Values: map[string]string{
				...
				"orchestration.clusterTopology.zones[0].name":             "region-a",
				"orchestration.clusterTopology.zones[0].numberOfBrokers":  "2",
				"orchestration.clusterTopology.zones[0].numberOfReplicas": "1",
				"orchestration.clusterTopology.zones[0].priority":         "100",
				"orchestration.clusterTopology.zones[1].name":             "region-a",
				"orchestration.clusterTopology.zones[1].numberOfBrokers":  "2",
				"orchestration.clusterTopology.zones[1].numberOfReplicas": "1",
				"orchestration.clusterTopology.zones[1].priority":         "50",
				"orchestration.profiles.broker":                       "true",
		...
		},
		{
			...
			Values: map[string]string{
				...
				"orchestration.clusterTopology.zone":                      "region-a",
				"orchestration.clusterTopology.zones[0].name":             "region-a",
				"orchestration.clusterTopology.zones[0].numberOfBrokers":  "1",
				"orchestration.clusterTopology.zones[0].numberOfReplicas": "3",
				"orchestration.clusterTopology.zones[0].priority":         "100",
				"orchestration.profiles.broker":                       "true",
			},
			Expected: map[string]string{
		...
		},
		{
			...
			Values: map[string]string{
				...
				"orchestration.clusterTopology.zones[0].numberOfBrokers":  "1",
				"orchestration.clusterTopology.zones[0].numberOfReplicas": "1",
				"orchestration.clusterTopology.zones[0].priority":         "0",
				"orchestration.profiles.broker":                       "true",
			},
			Verifier: func(t *testing.T, _ string, err error) {
				s.Require().ErrorContains(err, "/orchestration/clusterTopology/zones/0/priority': minimum: got 0, want 1")
			},
		},
		{
			...
			Values: map[string]string{
				"orchestration.clusterTopology.mode":                      "zoned",
				"orchestration.clusterTopology.zone":                      "region-c",
				"orchestration.clusterTopology.zones[0].name":             "region-a",
				"orchestration.clusterTopology.zones[0].numberOfBrokers":  "2",
				"orchestration.clusterTopology.zones[0].numberOfReplicas": "2",
				"orchestration.clusterTopology.zones[0].priority":         "100",
			...
			Expected: map[string]string{
				"ERROR": "orchestration.clusterTopology.zone \"region-c\" is not declared in orchestration.clusterTopology.zones",
			},
		},
		{
			...
			Expected: map[string]string{
				"ERROR": "orchestration.clusterTopology.zone must name the zone this release is deployed to",
			},
		},
	}
