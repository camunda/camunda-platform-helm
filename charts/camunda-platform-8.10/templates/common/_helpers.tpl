...
{{ .annotation }}: {{ get $data .certKey | sha256sum }}
{{ end -}}
{{- end -}}
...
{{- include "camundaPlatform.tlsChecksumAnnotation" (dict
    ...
    "secretName" $rest.secret.existingSecret
...
{{- include "camundaPlatform.tlsChecksumAnnotation" (dict
    ...
    "secretName" $grpc.secret.existingSecret
    "certKey" $grpc.secret.existingSecretKey) -}}
{{- end -}}
{{- end -}}
{{- end -}}
...
{{- $type := $c.secret.type | default "pkcs12" -}}
{{- $key := $c.secret.existingSecretKey -}}
{{- if and (eq $type "pem") (eq $key "keystore.p12") -}}
tls.crt
{{- else -}}
{{ $key }}
...
{{- $c := .Values.global.tls.connectors -}}
{{- if and $c.enabled $c.secret.existingSecret -}}
{{- include "camundaPlatform.tlsChecksumAnnotation" (dict
    "context" .
    "annotation" "checksum/connectors-tls"
    "secretName" $c.secret.existingSecret
...
{{/*
connectorsProxyVerifyAnnotations
Renders the NGINX upstream-TLS-verification annotations for the Connectors
ingress based on global.tls.connectors.proxyVerify. Returns nothing when
proxyVerify.enabled is false or caSecret.existingSecret is empty.

Connectors in 8.10 is exposed via Gateway API `HTTPRoute`, not via an NGINX
