...
func (s *deploymentTemplateTest) TestContainerSetImageNameSubChart() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"connectors.image.repository": "camunda/connectors-test",
	...
	s.Require().Equal(container.Image, "subchart.custom.registry.io/camunda/connectors-test:snapshot")
}
...
func (s *deploymentTemplateTest) TestContainerOverwriteImageTag() {
	...
	// then
	expectedContainerImage := "camunda/connectors:a.b.c"
...
func (s *deploymentTemplateTest) TestContainerShouldDisableOperateIntegration() {
	...
	for _, envvar := range env {
		s.Require().NotEqual("CAMUNDA_CONNECTORS_IDENTITY_ISSUER_URL", envvar.Name)
		s.Require().NotEqual("CAMUNDA_CONNECTORS_IDENTITY_ISSUER_BACKEND_URL", envvar.Name)
		s.Require().NotEqual("CAMUNDA_CONNECTORS_IDENTITY_CLIENT_ID", envvar.Name)
		s.Require().NotEqual("CAMUNDA_CONNECTORS_IDENTITY_CLIENT_SECRET", envvar.Name)
	}
}
...
func (s *deploymentTemplateTest) TestContainerShouldSetOperateIdentitySecretValue() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"global.identity.auth.connectors.existingSecret": "secretValue",
		},
		KubectlOptions: k8s.NewKubectlOptions("", "", s.namespace),
		ExtraArgs:      map[string][]string{"template": {"--debug"}, "install": {"--debug"}},
	}
	...
	s.Require().Contains(env,
		corev1.EnvVar{
			Name: "CAMUNDA_CONNECTORS_IDENTITY_CLIENT_SECRET",
...
func (s *deploymentTemplateTest) TestContainerShouldSetOperateIdentitySecretViaReference() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"global.identity.auth.connectors.existingSecret.name": "ownExistingSecret",
		},
		KubectlOptions: k8s.NewKubectlOptions("", "", s.namespace),
		ExtraArgs:      map[string][]string{"template": {"--debug"}, "install": {"--debug"}},
	}
...
func (s *deploymentTemplateTest) TestContainerShouldSetTheRightKeycloakServiceUrl() {
	...
	s.Require().Contains(env,
		corev1.EnvVar{
			Name:  "CAMUNDA_CONNECTORS_IDENTITY_ISSUER_BACKEND_URL",
...
func (s *deploymentTemplateTest) TestContainerStartupProbe() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"connectors.startupProbe.probePath":           "/healthz",
...
func (s *deploymentTemplateTest) TestContainerLivenessProbe() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"connectors.livenessProbe.probePath":           "/healthz",
