...
func (s *restapiDeploymentTemplateTest) TestContainerSetExtraVolumes() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"web-modeler.enabled":                                       "true",
			"web-modeler.restapi.extraVolumes[0].name":                  "extraVolume",
			"web-modeler.restapi.extraVolumes[0].configMap.name":        "otherConfigMap",
			"web-modeler.restapi.extraVolumes[0].configMap.defaultMode": "744",
		},
		KubectlOptions: k8s.NewKubectlOptions("", "", s.namespace),
	}
...
func (s *restapiDeploymentTemplateTest) TestContainerSetExtraVolumeMounts() {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"web-modeler.enabled":                                "true",
			"web-modeler.restapi.extraVolumeMounts[0].name":      "otherConfigMap",
			"web-modeler.restapi.extraVolumeMounts[0].mountPath": "/usr/local/config",
		},
		KubectlOptions: k8s.NewKubectlOptions("", "", s.namespace),
	}
