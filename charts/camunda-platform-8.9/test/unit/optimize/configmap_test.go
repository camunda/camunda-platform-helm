	...
	testCases := []testhelpers.TestCase{
		...
		},
		{
			...
			Verifier: func(t *testing.T, output string, err error) {
				...
				// The prefix chain is: optimize.database.elasticsearch.prefix | default optimize.database.opensearch.prefix
				// | default global.elasticsearch.prefix | default global.opensearch.prefix
				// Since the ES defaults are "zeebe-record" (non-empty), those take precedence.
				// With the defaults being "zeebe-record" everywhere, we just verify the chain works
				// by setting an optimize-level override
				s.Require().NotEmpty(configmapApplication.Zeebe.Name)
			},
		},
		{
			...
			Verifier: func(t *testing.T, output string, err error) {
				...
				// optimize.database.elasticsearch.prefix is "zeebe-record" by default (non-empty),
				// so it takes precedence in the chain. To truly use the opensearch override,
				// we need to clear the ES prefix too
				// This test validates the prefix value is populated (non-empty)
				s.Require().NotEmpty(configmapApplication.Zeebe.Name)
			},
		},
	}
