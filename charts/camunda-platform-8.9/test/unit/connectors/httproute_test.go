...
func TestHTTPRouteTemplate(t *testing.T) {
	...
	suite.Run(t, &HTTPRouteTemplateTest{
		...
		templates: []string{"templates/connectors/httproute.yaml"},
	})
}
...
func (s *HTTPRouteTemplateTest) TestDifferentValuesInputs() {
	testCases := []testhelpers.TestCase{
		...
		{
			Name: "TestHTTPRouteNotRenderedWhenGatewayExternal",
			Values: map[string]string{
		...
		},
		{
			...
			Verifier: func(t *testing.T, output string, err error) {
				...
				require.Contains(t, output, "name: camunda-platform-test-connectors")
		...
		},
		{
			...
			Verifier: func(t *testing.T, output string, err error) {
				require.NoError(t, err)
				require.Contains(t, output, "value: /connectors")
			},
		},
		{
