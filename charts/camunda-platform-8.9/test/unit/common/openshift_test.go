...
// TestAllOpenShiftFeaturesEnabled verifies that all three OpenShift features work
// together: security context adaptation, ingress TLS auto-management, and
// Elasticsearch sysctlImage disabling.
func (s *OpenShiftCombinedFeaturesTest) TestAllOpenShiftFeaturesEnabled() {
	testCases := []testhelpers.TestCase{
		...
		{
			Name: "ElasticsearchSysctlImageDisabledWithOpenShiftForce",
			Values: map[string]string{
				"global.compatibility.openshift.adaptSecurityContext": "force",
				"elasticsearch.enabled":                               "true",
			},
			...
			Verifier: func(t *testing.T, output string, err error) {
				require.NoError(t, err,
					"Chart should render successfully; z_compatibility_helpers.tpl disables elasticsearch sysctlImage")
			},
		},
	}
