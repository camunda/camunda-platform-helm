...
func (s *shouldAutogenerateSecretTest) TestShouldAutogenerateSecretWithExistingSecretString() {
	// Test case: component has secret.existingSecret pointing to autogen secret
	config := map[string]string{
		"identity.firstUser.secret.existingSecret":    "camunda-credentials",
		"identity.firstUser.secret.existingSecretKey": "identity-firstuser-password",
	...
	s.Require().True(result, "identity-firstuser-password should be generated when identity.firstUser.secret.existingSecret points to autogen secret")
}
...
func (s *shouldAutogenerateSecretTest) TestShouldAutogenerateSecretWithExistingSecretObject() {
	...
	s.Require().True(result, "identity-optimize-client-token should be generated when global.identity.auth.optimize.secret.existingSecret points to autogen secret")
}
...
func (s *shouldAutogenerateSecretTest) TestShouldAutogenerateSecretWithNoSecretConfig() {
	// Test case: component has no secret configuration but existingSecretKey is set - should autogenerate
	config := map[string]string{
...
func (s *shouldAutogenerateSecretTest) TestShouldAutogenerateSecretWithCustomSecretName() {
	...
	config := map[string]string{
		...
		"identity.firstUser.secret.existingSecret":    "my-custom-autogen-secret",
		"identity.firstUser.secret.existingSecretKey": "identity-firstuser-password",
		"global.identity.auth.enabled":                "true",
	}
