...
{{- else -}}
  ...
  {{- if or 
      (and $config.existingSecret (eq (toString $config.existingSecret) (toString $autogenSecretName)))
      (and $config.existingSecret.name (eq (toString $config.existingSecret.name) (toString $autogenSecretName)))
      (and $config.secret.existingSecret (eq (toString $config.secret.existingSecret) (toString $autogenSecretName)))
      (and $config.auth.existingSecret (eq (toString $config.auth.existingSecret) (toString $autogenSecretName)))
      (and $config.postgresql.auth.existingSecret (eq (toString $config.postgresql.auth.existingSecret) (toString $autogenSecretName)))
  -}}
    {{- $result = "true" -}}
  {{- end -}}
{{- end -}}
