...
{{- else -}}
  ...
  {{- if and $config.existingSecret (eq $config.existingSecret $autogenSecretName) -}}
    {{/* Legacy format points to autogen secret -> autogenerate */}}
    {{- $result = "true" -}}
  {{- else if and $config.secret $config.secret.existingSecret (eq $config.secret.existingSecret $autogenSecretName) -}}
