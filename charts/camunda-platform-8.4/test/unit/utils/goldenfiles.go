...
func TestGoldenTemplates(t *testing.T, chartPath string, componentName string, templateNames []string, ignoredLines []string, setValues map[string]string, extraHelmArgs []string) {
	for _, templateName := range templateNames {
		...
		// Identity is a subchart in 8.4 and uses a different path
		if componentName == "identity" {
			templatePath = fmt.Sprintf("charts/%s/templates/%s.yaml", componentName, templateName)
		} else {
			templatePath = fmt.Sprintf("templates/%s/%s.yaml", componentName, templateName)
		}
