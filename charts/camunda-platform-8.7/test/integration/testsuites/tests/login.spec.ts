...
async function fetchToken(id: string, sec: string, api: APIRequestContext) {
  ...
  expect(r.ok(), `Failed to get token for client_id=${id}: ${r.status()}`).toBeTruthy();
...
test.describe('Camunda core', () => {
  ...
  test('M2M tokens', async () => {
    for (const [id, sec] of Object.entries(config.secrets)) {
      ...
      await expect(fetchToken(id, sec, api)).resolves.toMatch(/^[\w-]+\.[\w-]+\.[\w-]+$/);
    }
  });
  ...
  for (const [name, url] of Object.entries({
    ...
    test(`Login page: ${name}`, async () => {
      const r = await api.get(`${url}${config.loginPath[name]}`, { timeout: 45_000 });
      expect(r.ok(), `Login page failed for ${name}: ${r.status()}`).toBeTruthy();
      expect(await r.text(), `Login page for ${name} contains error`).not.toMatch(/error/i);
    });
  }

  test('Connectors inbound page', async () => {
    expect((await api.get(config.base.connectors, { timeout: 45_000 })).ok(), 'Connectors inbound page failed').toBeTruthy();
  });
  ...
  for (const [label, url, method, body] of [
    ...
    ['Operate defs', `${config.base.operate}/v1/process-definitions/search`, 'POST', '{}'],
    ['Tasklist tasks', `${config.base.tasklist}/graphql`, 'POST', '{"query":"{tasks(query:{}){id name}}"}'],
  ] as const) {
    test(`API: ${label}`, async () => {
      ...
      expect(r.ok(), `API call failed for ${label}: ${r.status()}`).toBeTruthy();
    });
  }
  ...
  test('WebModeler login page', async () => {
    ...
    expect(await r.text(), 'WebModeler login page contains error').not.toMatch(/error/i);
  });
  ...
  test('Zeebe status (gRPC)', async () => {
    ...
    const out = execFileSync('zbctl', ['status',
      '--clientCache','/tmp/zeebe','--clientId',config.venomID,'--clientSecret',config.venomSec,
      '--authzUrl',config.authURL,'--address',config.base.zeebeGRPC,
      ...extra,
    ],{ encoding:'utf-8' });
    expect(out, 'zbctl status output missing Leader, Healthy').toMatch(/Leader, Healthy/);
    expect(out, 'zbctl status output contains Unhealthy').not.toMatch(/Unhealthy/);
  });
  ...
  test('Zeebe topology (REST)', async () => {
    ...
    expect(await r.json(), 'Zeebe topology response missing brokers').toHaveProperty('brokers');
  });
  ...
  for (const [bpmnId, label] of [['it-test-process','Basic'],['test-inbound-process','Inbound']] as const) {
    test(`Process visible: ${label}`, async () => {
      const r = await api.post(`${config.base.operate}/v1/process-definitions/search`, {
        data:'{}',
        headers:{ Authorization:`Bearer ${venomJWT}`,'Content-Type':'application/json' },
      });
      expect(r.ok(), `Process visibility check failed for ${label}: ${r.status()}`).toBeTruthy();
      const data = await r.json();
      const ids = (data.items as Array<{ bpmnProcessId: string }>).map(i => i.bpmnProcessId);
