...
{{/*
Generate environment variable definitions for a given document store type.
Parameters:
  Type  - The document store type (e.g. "aws", "gcp", "inmemory", "localstorage").
  Values - The global values object.
Example usage:
  {{ include "camundaPlatform.documentStoreEnv" (dict "Type" "aws" "Values" .Values) }}
This helper builds a block of environment variable definitions based on the configuration
for the specified document store type.
*/}}
{{- define "camundaPlatform.documentStoreEnv" -}}
  {{- $typeKey := lower .Type -}}
  {{- $store := index .Values.global.documentStore.type $typeKey -}}
  {{- $prefix := upper (default (upper .Type) $store.storeId) -}}
  {{- if eq $typeKey "aws" -}}
    {{- if $store.enabled }}
- name: AWS_REGION
  value: {{ $store.region | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_BUCKET" $prefix | quote }}
  value: {{ $store.bucket | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_BUCKET_PATH" $prefix | quote }}
  value: {{ $store.bucketPath | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_BUCKET_TTL" $prefix | quote }}
  value: {{ $store.bucketTtl | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_CLASS" $prefix | quote }}
  value: {{ $store.class | quote }}
    {{- if $store.existingSecret }}
- name: AWS_ACCESS_KEY_ID
  valueFrom:
    secretKeyRef:
      name: {{ $store.existingSecret | quote }}
      key: {{ $store.accessKeyIdKey | quote }}
- name: AWS_SECRET_ACCESS_KEY
  valueFrom:
    secretKeyRef:
      name: {{ $store.existingSecret | quote }}
      key: {{ $store.secretAccessKeyKey | quote }}
    {{- end }}
    {{- end }}
  {{- else if eq $typeKey "gcp" -}}
    {{- if $store.enabled }}
- name: {{ printf "DOCUMENT_STORE_%s_BUCKET" $prefix | quote }}
  value: {{ $store.bucket | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_CLASS" $prefix | quote }}
...
- name: GOOGLE_APPLICATION_CREDENTIALS
  value: {{ printf "%s/%s" (default "/var/secrets/gcp" $store.mountPath) (default "service-account.json" $store.fileName) | quote }}
    {{- end }}
  {{- else if eq $typeKey "inmemory" -}}
    {{- if $store.enabled }}
- name: {{ printf "DOCUMENT_STORE_%s_CLASS" $prefix | quote }}
  value: {{ $store.class | quote }}
    {{- end }}
  {{- else if eq $typeKey "localstorage" -}}
    {{- if $store.enabled }}
- name: {{ printf "DOCUMENT_STORE_%s_CLASS" $prefix | quote }}
  value: {{ $store.class | quote }}
- name: {{ printf "DOCUMENT_STORE_%s_PATH" $prefix | quote }}
  value: {{ $store.path | quote }}
    {{- end }}
  {{- end }}
{{- end }}
